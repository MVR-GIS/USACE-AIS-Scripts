...
# author: "ryan.benac"
# date: 1/31/2025
# excludes ECHQ data and filters to only Rock Island District Projects
...
print("Requesting MVR borings...")
all_borings <- get_multi_project_borings("Rock Island")
...
upload_shapefile <- "C:/Workspace/AUTOMATED_SCRIPTS/OpenGround/OUTPUT GEOJSON/openGround_MVR_borings.shp"
st_write(sf_data, upload_shapefile, quiet = TRUE, append=FALSE)

################################################################# CREDIT TO BARRIE C (CEMVR EC-TG)
### Import necessary python packages and authenticate to portal
library(reticulate)
...
print("Connecting to MVR Portal...")
gis <- arcgis$GIS("https://geoportal.mvr.usace.army.mil/b5portal/")

print(paste("Connected as:", gis$properties$user$username))


################################################################# 
### Find the item id for the layer you created that you want to update
# MVR Openground borings
item_id <- "1d30a71e4357409a96afb8e9695b0db7"
feature_layer_item <- gis$content$get(item_id)

print(paste0("Using item ", feature_layer_item$title," at https://geoportal.mvr.usace.army.mil/b5portal/home/item.html?id=", item_id))
feature_layers <- feature_layer_item$layers
feature_layer <- feature_layers[[1]]  # Get the first layer (note the double brackets in R)

feature_layer_url <- feature_layer$url

# Check if the shapefile exists
if (!os$path$exists(upload_shapefile)) {
  stop(paste("Shapefile not found at", upload_shapefile))
}

### 6. Use the arcpy.management.append function to update your layer with the new data
print("Updating item...")
# Append the data to the feature service
arcpy$management$Append(
  inputs=upload_shapefile,
  target=feature_layer_url,
  schema_type="NO_TEST",  # Use 'TEST' to validate schema first
  field_mapping="",
  subtype="",
  expression=""
)
...
### 7. Yay you did it
print(paste("Successfully appended data to feature service:", feature_layer_item$title))
